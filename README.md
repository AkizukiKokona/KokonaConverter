...
- **FBX 7.4.0 ASCII** 输出（`FBXVersion: 7400`），UE4.27 官方推荐版本

### 2. 保留的内容
| 类别 | 说明 |
|---|---|
| 骨骼层级 | 完整保留父子关系、局部平移/旋转、固定轴、局部坐标系、继承旋转/平移、外部亲 |
...
- 转换时把找到的贴图复制到 FBX 同级 `textures/` 子目录，FBX 中以相对路径引用，UE4 导入时能直接定位
...
3. 贴图：FBX 同级的 `textures/` 文件夹会被自动扫描；如未自动导入，手动 Import 该文件夹即可
...
| 贴图丢失 | 检查 `textures/` 是否在 FBX 旁边；中文路径是否完整 |
...
├── fbx_writer.py        # FBX 7.4.0 ASCII 写入器（零依赖）
├── texture_utils.py     # 贴图路径解析 / 中文路径 / tex 子目录识别
├── convert.py           # 转换编排器 + JSON 元数据 sidecar
├── main.py              # 入口：tkinter GUI + CLI
...
### 运行依赖
- **Python 3.8+**（仅用标准库：`struct`、`os`、`sys`、`json`、`math`、`argparse`、`tkinter`、`threading`、`dataclasses`、`typing`）
...
### 为什么用 ASCII FBX 而不是二进制？
- ASCII 便于调试和肉眼检查
- UE4.27 对 ASCII 和二进制 FBX 支持度相同
- 7.4.0 是 UE4.27 自动导入 FBX 时识别最稳定的版本
- 写入器完全可控，不依赖 Autodesk SDK
...
- FBX ASCII 格式：参考 Autodesk FBX SDK 文档与开源项目 `fbx2json` / `json2fbx` 的格式样例
